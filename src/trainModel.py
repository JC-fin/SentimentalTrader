...
class Trainer:
    def __init__(self, ticker):
        ...
        wrapper = tdw()
        self.stock_data = wrapper.time_series([ticker], interval='15min', start_date='2010-01-01', end_date='2020-10-12').sort_values('datetime', ascending=True).set_index('datetime')
        self.macd_data = wrapper.macd(ticker, interval='15min', start_date='2010-01-01', end_date='2020-10-12').sort_values('datetime', ascending=True).set_index('datetime')
    ...
    def generateTrainData(self, seq_len):
        for i in range((len(self.train_data)//seq_len)*seq_len - seq_len - 1):
            x = np.array(self.raw_data.iloc[i: i + seq_len, [3, 4, 5, 6]])
            y = np.array([self.raw_data.iloc[i + seq_len + 1, 3]], np.float64)
            self.input_train.append(x)
            self.output_train.append(y)
        self.X_train = np.array(self.input_train).reshape((len(self.input_train), 60, 4)) / np.array([250, 10000000, 1, 1])
    ...
    def generateTestData(self, seq_len):
        for i in range((len(self.test_data)//seq_len)*seq_len - seq_len - 1):
            x = np.array(self.test_data.iloc[i: i + seq_len, [3, 4, 5, 6]])
            y = np.array([self.test_data.iloc[i + seq_len + 1, 3]], np.float64)
            self.input_test.append(x)
            self.output_test.append(y)
        self.X_test = np.array(self.input_test).reshape((len(self.input_test), 60, 4)) / np.array([250, 10000000, 1, 1])
